...
def collect_es6_sources(ctx):
  ...
  for dep in ctx.attr.deps:
    if hasattr(dep, "typescript"):
      ...
      for es6_source in dep.files:
        module_path = es6_source.short_path[es6_source.short_path.rfind('node_modules/'):]

        rerooted_es6_source = ctx.actions.declare_file("%s.es6/%s" % (ctx.label.name, module_path))
        ctx.actions.expand_template(
          output = rerooted_es6_source,
          template = es6_source,
          substitutions = {}
